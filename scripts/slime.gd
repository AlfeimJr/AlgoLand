...
class_name SlimeEnemy  # Nome atualizado para evitar conflito com a classe global "Slime"
...
func _ready() -> void:
	...
	if animation_tree:
		...
		_set_animation("run")  # Sempre usa o estado "run"

	# Configure o Area2D (usado para detectar o player, por exemplo)
...
func take_damage(damage: int, knockback_force: Vector2 = Vector2.ZERO) -> void:
	...
	# Usa sempre a animação "run" mesmo ao tomar dano
	_set_animation("run")
	
	# Se nenhum knockback for fornecido, aplica um padrão (empurra para longe do player)
	if knockback_force == Vector2.ZERO:
		knockback_force = (position - player.position).normalized() * 300.0

	# Aplica knockback imediatamente, reiniciando mesmo se em estado de dano
	is_recoiling = true
